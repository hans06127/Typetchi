(() => {
  ...
  function injectTypetchiRoot() {
    ...
    style.textContent = `
      ...
      .typetchi-stage { display: grid; place-items: center; min-height: 136px; }
      .typetchi-pet { position: relative; width: 184px; height: 136px; transform-origin: center bottom; }
  ...
  function svgPetRenderer(stageId, compact = false) {
    const pet = document.createElement('div');
    pet.className = `typetchi-pet ${stageId} ${animationState}${compact ? ' compact' : ''}`;
    pet.dataset.renderer = RENDERER_TYPE;
  ...
  function createPetElement(stageId, compact = false) {
    return svgPetRenderer(stageId, compact);
  }
  function render() {
    if (!appRoot) return;
